...
//GLOBALS
$GLOBALS['DUPLICATOR_INSTALLER_VERSION'] =  '0.2.8';
...
<head>
	...
	<style type="text/css">
		...
		div#content {	border:1px solid #CDCDCD;  width:750px; height:790px; margin:auto; margin-top:18px; border-radius:5px}
		...
		div.final-msg{height:350px;overflow-y:scroll;border:1px solid #CDCDCD; padding:8px;font-size:12px;line-height:22px !important;background-color:#efefef; border-radius:5px;}
...
<div id="content">
	...
	if ($action == 'process') {
		...
		mysql_query("DELETE FROM `wpplug_duplicator`");
		
		//Update site title
		$site_title = mysql_real_escape_string($_POST['site_title']);
		mysql_query("SET wait_timeout = {$GLOBALS['MAX_TIME']}");
		mysql_query("SET NAMES 'utf8' ");
		mysql_query("UPDATE `wpplug_options` SET option_value = '{$site_title}' WHERE option_name = 'blogname' ");
		
		//Validate the core tables have content
		$table_users	= dinstaller_table_row_count("wpplug_users");
		$table_posts	= dinstaller_table_row_count("wpplug_posts");
		$table_opts		= dinstaller_table_row_count("wpplug_options");
		...
		if ($GLOBALS["REMOVE_DBCACHE"]) {
			...
			mysql_query("DELETE FROM `wpplug_options` WHERE `option_name` LIKE ('_transient%')");
			mysql_query("DELETE FROM `wpplug_options` WHERE `option_name` LIKE ('_site_transient%')");
		}
		...
		<form id="form-installer" action="install.php" method="post" >
			...
			<table width="100%" border="0" cellspacing="2" cellpadding="2" class="table-inputs">
				...
				<tr valign="top">
					...
					<td><input type="text" name="current_url" id="current_url" value="" readonly="true"  class="readonly" /></td>
				</tr>
				<tr>
					<td>Install Url</td>
			...
			<table width="100%" border="0" cellspacing="2" cellpadding="2"  class="table-inputs">
				...
				<tr><td colspan="2">
					<div style="margin:-5px 0px 0px 140px; font-size:12px;">
						...
						<input type="checkbox" name="dbcharvalid" id="dbcharvalid" value="1" /> <label for="dbcharvalid">Replace Invalid Characters</label>
					</div>
				</td></tr>
